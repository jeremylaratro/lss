...
class SecurityControlPanel:
    ...
    def create_alerts_tab(self):
        """Instantiate the extracted AlertsTab and add it to the notebook."""
        self.alerts_tab = AlertsTab(self.notebook, self)
        self.notebook.add(self.alerts_tab.frame, text="󰀦 Alerts")

    def create_traffic_tab(self):
        """Instantiate the extracted TrafficTab and add it to the notebook."""
        self.traffic_tab = TrafficTab(self.notebook, self)
        self.notebook.add(self.traffic_tab.frame, text="󰖟 Traffic")
    ...
    def create_dns_tab(self):
        """Instantiate the extracted DNSTab and add it to the notebook."""
        self.dns_tab = DNSTab(self.notebook, self)
        self.notebook.add(self.dns_tab.frame, text="󰇖 DNS")
    ...
    def create_clamav_quarantine_tab(self):
        """Instantiate the extracted QuarantineTab and add it to the notebook."""
        self.quarantine_tab = QuarantineTab(self.notebook, self)
        self.notebook.add(self.quarantine_tab.frame, text="󰀦 Quar")
...
"""
    ...
    def refresh_alerts(self):
        """Delegate to the extracted AlertsTab (kept for auto-refresh dispatch)."""
        if getattr(self, 'alerts_tab', None) is not None:
    ...
    def filter_alerts_treeview(self, search_text):
        """Delegate global-search filtering to the extracted AlertsTab."""
        if getattr(self, 'alerts_tab', None) is not None:
            self.alerts_tab.filter_alerts_treeview(search_text)
    ...
    def _update_filter_count(self):
        """Delegate to the extracted AlertsTab (called once at startup)."""
        if getattr(self, 'alerts_tab', None) is not None:
            self.alerts_tab._update_filter_count()
    ...
    def refresh_traffic(self):
        """Delegate to the extracted TrafficTab (kept for auto-refresh dispatch)."""
        if getattr(self, 'traffic_tab', None) is not None:
            self.traffic_tab.refresh()
    ...
    def refresh_dns(self):
        """Delegate to the extracted DNSTab (kept for auto-refresh dispatch)."""
        if getattr(self, 'dns_tab', None) is not None:
            self.dns_tab.refresh()
    ...
    def refresh_quarantine(self):
        """Delegate to the extracted QuarantineTab (kept for auto-refresh dispatch)."""
        if getattr(self, 'quarantine_tab', None) is not None:
            self.quarantine_tab.refresh()
